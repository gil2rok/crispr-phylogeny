...
# NOTE: edited to set minimum to some small value to prevent divide-by-zero
EPSILON = 1e-3
...
class Hyperboloid:
    ...
    def random_unit_tangent(self, point, orthogonal_to=None):
        ...
        return trandom / (tangent_norm(trandom) + EPSILON)
    ...
    def embed_tree(self, tree):
        ...
        for node in tree.preorder_internal_node_iter():
            ...
            if node != tree.seed_node:  # seed_node means root node
                ...
                tangent1 = tparent / (tangent_norm(tparent) + EPSILON)  # make unit vector for calculations
